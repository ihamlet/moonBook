<template>
  <div class="container" id='media' @click="toArticle" :class="$route.name == 'article'?'article':''">
    ...
    <div class="text" ref='textContent' v-if='item.details' v-line-clamp:20="type == 'card'?2:0" :class="item.template_id == 0?'content':''" v-html='`<span class="theme-color tags">${item.tags?`#${item.tags}#`:""}</span>${item.details}`'></div>
    <!-- 视频  -->
